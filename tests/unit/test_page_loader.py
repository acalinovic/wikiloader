...
def test_page_loader_article_not_found(page_loader):
    ...
    print(page_loader.get_current_source())
    print(f"Soup {'doesn\'t if page_loader.get_current_soup() else ''}exists")
    assert page_loader.get_current_soup().find("div", attrs={"class": "noarticletext mw-content-ltr"}) is None
